...
int main(int argc, char* argv[])
{
    ...
    while(appletMainLoop()) {
        ...
        if(kDown & HidNpadButton_L) {
            showKeyboard(url, "Choose URL", "Enter a URL", "Go", "http://192.168._._:5000");
            break;
        } else if(kDown & HidNpadButton_R) {
            char defUrl[0xc00] = {0};
            char fURL[0xc00] = {0};
            FILE* dUCheck = fopen("sdmc:/switch/webdeckUrl.txt", "r");
            if (dUCheck != NULL) {
            ...
            fclose(dUCheck);
            if(showKeyboard(defUrl, "Set default URL", "Enter A URL", "Set", "http://192.168._._:5000") == 0) {
                FILE* dUFile = fopen("sdmc:/switch/webdeckUrl.txt", "w+");
                fprintf(dUFile, "%s", defUrl);
                fclose(dUFile);
            }
        } else if(kDown & HidNpadButton_X) {
            remove("sdmc:/switch/webdeckUrl.txt");
    ...
    FILE* dUFile = fopen("sdmc:/switch/webdeckUrl.txt", "r");
    char fURL[0xc00] = {0};
    if(dUFile != NULL) {
